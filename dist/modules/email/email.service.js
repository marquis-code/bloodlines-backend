...
let EmailService = EmailService_1 = class EmailService {
    ...
    async sendEmailVerification(email, token) {
        const appUrl = this.configService.get("API_BASE_URL") || "http://localhost:3005";
        const verificationLink = `${appUrl}/auth/verify-email?token=${token}`;
        const html = this.brandWrapper("Verify Your Email", `
    ...
    async sendWelcomeEmail(email, fullName) {
        const html = this.brandWrapper("Welcome to Bloodlines!", `
    ...
    async sendPasswordReset(email, token) {
        const appUrl = this.configService.get("CORS_ORIGIN") || "http://localhost:3000";
        const resetLink = `${appUrl}/reset-password?token=${token}`;
        const html = this.brandWrapper("Reset Your Password", `
    ...
    async sendPasswordResetSuccess(email, fullName) {
        const html = this.brandWrapper("Password Changed", `
      <p>Hi <strong>${fullName}</strong>,</p>
    ...
    async sendRoleUpgradeNotification(email, userName, requestedRole) {
        const html = this.brandWrapper("Role Upgrade Request Received", `
    ...
    async sendRoleUpgradeApproved(email, userName, newRole) {
        const html = this.brandWrapper("Role Upgrade Approved! 🎉", `
      <p>Hi <strong>${userName}</strong>,</p>
    ...
    async sendRoleUpgradeRejected(email, userName, requestedRole, reason) {
        const html = this.brandWrapper("Role Upgrade Update", `
      <p>Hi <strong>${userName}</strong>,</p>
    ...
    async sendNotification(email, title, body, data) {
        let extraContent = "";
      ...
      `;
        ...
        const html = this.brandWrapper(title, `
      <p>${body}</p>
      ${extraContent}
